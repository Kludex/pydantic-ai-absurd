...
from pydantic_ai_absurd import AbsurdMCPToolset
...
async def test_server_access(absurd: AsyncAbsurd) -> None:
    toolset = AbsurdMCPToolset(_make_server(), step_name_prefix='a')
...
async def test_id_passthrough(absurd: AsyncAbsurd) -> None:
    inner = _make_server()
    toolset = AbsurdMCPToolset(inner, step_name_prefix='a')
...
async def test_get_tools_and_call_tool_are_checkpointed(absurd: AsyncAbsurd) -> None:
    toolset = AbsurdMCPToolset(_make_server(), step_name_prefix='a')
    absurd.register_task(name='noop')(_noop)
...
async def test_get_tools_without_context_passes_through(absurd: AsyncAbsurd) -> None:
    toolset = AbsurdMCPToolset(_make_server(), step_name_prefix='a')
    tools = await toolset.get_tools(_run_context())
    assert 'add' in tools


async def test_get_instructions_without_context_passes_through(absurd: AsyncAbsurd) -> None:
    toolset = AbsurdMCPToolset(_make_server(), step_name_prefix='a')
...
async def test_get_instructions_inside_context_returns_none_when_disabled(absurd: AsyncAbsurd) -> None:
    toolset = AbsurdMCPToolset(_make_server(), step_name_prefix='a')
    absurd.register_task(name='noop')(_noop)
...
async def test_get_instructions_inside_context_with_include(absurd: AsyncAbsurd) -> None:
    # Enable include_instructions so the step dispatch path runs.
...
def _make_stdio_server() -> MCPToolset[None]:
    return MCPToolset(MCP_SCRIPT)


async def test_stdio_get_tools_and_call_tool_inside_context(absurd: AsyncAbsurd) -> None:
    server = _make_stdio_server()
    toolset = AbsurdMCPToolset(server, step_name_prefix='a')
    absurd.register_task(name='noop')(_noop)
...
async def test_stdio_get_tools_without_cache(absurd: AsyncAbsurd) -> None:
